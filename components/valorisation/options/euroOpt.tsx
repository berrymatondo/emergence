...
import { EuroSchema } from "@/lib/schemas";
import {
  computeEuroOptionDiscountCurve,
  computeGeneralEuropeanOption,
} from "@/lib/_optionActions";
import VolatilityCurve from "./Volatility";
import DividendCurve from "./dividend";
...
const initialVolatilityCurve: any[] = [{ id: 1, tenor: 0, rate: 0.0 }];
const initialDividendCurve: any[] = [{ id: 1, tenor: 0, rate: 0.0 }];
...
type EuroOptionProps = {
  countries: any;
  currencies: any;
};

const EuroOption = ({ countries, currencies }: EuroOptionProps) => {
  ...
  const [volatility, setVolatility] = useState(initialVolatilityCurve);
  const [dividend, setDividend] = useState(initialDividendCurve);
  ...
  const form = useForm<z.infer<typeof EuroSchema>>({
    resolver: zodResolver(EuroSchema),
    defaultValues: {
      optionMaturityDate: "2030-07-30",
      optionCurrency: "1",
      spot: "110",
      strike: "105",
      optionType: "put",
 ...
 */
  ...
  const procesForm = async (values: z.infer<typeof EuroSchema>) => {
    setLoading(true);
    ...
    const dcurve = await computeEuroOptionDiscountCurve(
      values,
      disc,
      yieldcurve,
      zcrates,
    ...
    //console.log("Value:", values.notional);

    if (dcurve?.data) setDisc(dcurve?.data);
    if (values?.strike) setStrike(+values?.strike);
    if (values?.notional) setNotional(+values?.notional);
    ...
    let tmp;
    const global = await computeGeneralEuropeanOption(
      values,
      dcurve?.data,
      volatility,
      dividend
    ...
    if (global?.data) {
      ...
      setDelta(global?.data?.delta);
      setOption(global?.data?.price);
  ...
  return (
    <GeneralLayout
      title="European Option Valuation"
      bred={<CustomBreadcrumb name="European Option Valuation" />}
    >
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem className=" w-1/2">
                              <FormLabel>{"Option Maturity Date"}</FormLabel>
                              <FormControl>
                                <Input
                                  {...field}
                    ...
                    <div className="flex justify-between items-center gap-4">
                      <FormField
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem className="w-1/2">
                              <FormLabel className="w-1/3">{"Model"}</FormLabel>
                              <Select
                                onValueChange={field.onChange}
                                defaultValue={field.value}
                              >
          ...
          <div className="md:flex md:items-center md:justify-around border rounded-xl mt-4 p-4 bg-sky-400/20 dark:bg-sky-400/30">
            ...
            <div className="flex md:flex-col justify-between items-center md:items-start gap-4">
              <div className="grid flex-1 auto-rows-min gap-0.5 mt-4">
                ...
                {strike ? (
                  <div className="flex items-baseline gap-1 text-2xl font-bold tabular-nums leading-none">
                    {((option * +notional) / +strike).toFixed(2)}
                  </div>
                ) : (
                  "-"
                )}
