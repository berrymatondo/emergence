...
import {
  computeAmoCommoYieldToMaturity,
  computeGeneralAmoCommoBond,
} from "@/lib/_commoActions";
import { ACommoSchema } from "@/lib/schemas";
import AmoCommoSched from "./amoCommoSched";
...
const AmoCommoBackBond = ({
  ...
  const procesForm = async (values: z.infer<typeof ACommoSchema>) => {
    ...
    if (global?.data) {
      ...
      for (
        let i = 0;
        i < global?.data?.cash_flow?.discounted_cash_flow?.length;
        i++
      ) {
        cashflowFin.push({
          gross: global?.data?.cash_flow?.cash_flow[i],
          date: global?.data?.cash_flow?.date[i],
          discounted: global?.data?.cash_flow?.discounted_cash_flow[i],
        });
      }
    ...
    const yieldToMaturit = await computeAmoCommoYieldToMaturity(
      values,
      tmp,
      dcurve?.data,
      commoForward,
...
export default AmoCommoBackBond;
