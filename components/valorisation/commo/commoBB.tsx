...
import { CommoSchema } from "@/lib/schemas";
import {
  computeCommoYieldToMaturity,
  computeGeneralCommoBond,
...
const CommoBackBond = ({
  ...
  const form = useForm<z.infer<typeof CommoSchema>>({
    resolver: zodResolver(CommoSchema),
    defaultValues: {
  ...
  const procesForm = async (values: z.infer<typeof CommoSchema>) => {
    setLoading(true);
    ...
    let tmp;
    const global = await computeGeneralCommoBond(
      values,
      dcurve?.data,
      commoForward
    );
    ...
    const yieldToMaturit = await computeCommoYieldToMaturity(
      values,
      tmp,
      dcurve?.data,
      commoForward
  ...
  return (
    <GeneralLayout
      ...
      bred={<CustomBreadcrumb name="Commodity Back Bond Valuation" />}
    >
                    ...
                    <div className="flex justify-between items-center gap-4">
                      <FormField
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem className="w-1/2">
                              <FormLabel>{"Fixed Coupon (%)"}</FormLabel>
                              <FormControl>
                                <Input
                                  {...field}
                                  placeholder="Entrer la valeur"
                      ...
                      <FormField
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem className="w-1/2">
                              <FormLabel>{"Coupon Max (%)"}</FormLabel>
                              <FormControl>
                                <Input
                                  {...field}
                                  placeholder="Entrer la valeur"
