...
const FloatingRateBond = ({ countries, currencies }: FloatingRateBondProps) => {
  ...
  const procesForm = async (values: z.infer<typeof FloatingSchema>) => {
    ...
    let tmp;
    const global = await computeGeneralFloatingBond(
      values,
      dcurve?.data,
      forwardrates
    );
    ...
    const yieldToMaturit = await computeFloatingYieldToMaturity(
      values,
      tmp,
      dcurve?.data,
      forwardrates
...
export default FloatingRateBond;
