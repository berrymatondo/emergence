...
import { getAllImpMatrix } from "@/lib/_matrix";
import ImpMatrixForm from "./impMatrixForm";
import DelImpMatrix from "./delImpMatrix";

const ImpMatrix = async () => {
    ...
    <div>
      <div className="bg-gray-500/10 dark:bg-teal-200/10   max-md:w-full  p-4 rounded-xl">
        <ScrollArea className="h-96">
          <ImpMatrixForm openDialog={false} type="A" />
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {imp?.data?.map((ic: any) => (
                <TableRow key={ic.id}>
                  ...
                  <TableCell className="flex  items-center gap-4 text-right  mx-0 px-0 hover:cursor-pointer">
                    <ImpMatrixForm openDialog={false} type="U" impIn={ic} />
                    <DelImpMatrix id={ic.id} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
